...
for ii = 1 : N
    for jj = 1 : N
        if ii ~= jj && ~blind_neighbors(ii, jj)
            distance = dist(ii, jj);
            if distance <= rr
                dxi(1:2, ii) = dxi(1:2, ii) + x(1:2, ii) - x(1:2, jj);
            end
        end
    end
end
...
for ii = 1 : N
    for jj = 1 : N
        if ii ~= jj && ~blind_neighbors(ii, jj)
            distance = dist(ii, jj);
            if distance <= ro && distance > rr
                dxi(1:2, ii) = dxi(1:2, ii) - x(1:2, jj) - (0.5 * x(1:2, ii));
            end
        end
    end
end
...
for ii = 1 : N
    for jj = 1 : N
        if ii ~= jj && ~blind_neighbors(ii, jj)
            distance = dist(ii, jj);
            if distance <= ra && distance > rr
                dxi(1:2, ii) = dxi(1:2, ii) + x(1:2, jj) - x(1:2, ii);
            end
        end
    end
end
