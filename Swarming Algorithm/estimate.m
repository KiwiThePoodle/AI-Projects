...
for ii = 1:1:N
    % calculate mean neighbor estimate and the standard deviation
    neighbor_estimates = [];
    for jj = 1:1:N
        if ((ii ~= jj) && neighbors(ii, jj))
            neighbor_estimates = [neighbor_estimates, temperature_est(jj,1)];
        end
    end
    mean_estimate = mean(neighbor_estimates);
    std_estimate = std(neighbor_estimates);
    for jj = 1:1:N
        ...
        if ((ii ~= jj) && neighbors(ii, jj)) % ii is not jj and jj is ii's neighbor
            if (abs(temperature_est(jj, 1) - mean_estimate) < 2 * std_estimate)
